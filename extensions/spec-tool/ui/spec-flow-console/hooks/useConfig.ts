...
interface UseConfigApiReturn {
  ...
  saveConfig: (config: { nextAiLabs?: NextAiLabsConfig; mem0: Mem0Config }) => Promise<void>;
...
export const useConfig = (): UseConfigApiReturn => {
  ...
  const fetchConfig = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/api/v1/config`);
  ...
  const saveConfig = async (config: { nextAiLabs?: NextAiLabsConfig; mem0: Mem0Config }) => {
    setIsLoading(true);
    setError(null);
    
    try {
      const response = await axios.put(`${baseUrl}/api/v1/config`, config);
      dispatch(setConfigSuccess(response.data));
      setIsLoading(false);
  ...
  const resetConfig = async () => {
    ...
    try {
      const response = await axios.post(`${baseUrl}/api/v1/config/reset`);
      dispatch(setConfigSuccess(response.data));
      setIsLoading(false);
      return response.data;
    } catch (err: any) {
  ...
  const saveLLMConfig = async (llmConfig: LLMProvider) => {
    ...
    try {
      const response = await axios.put(`${baseUrl}/api/v1/config/mem0/llm`, llmConfig);
  ...
  const saveEmbedderConfig = async (embedderConfig: EmbedderProvider) => {
    ...
    try {
      const response = await axios.put(`${baseUrl}/api/v1/config/mem0/embedder`, embedderConfig);
