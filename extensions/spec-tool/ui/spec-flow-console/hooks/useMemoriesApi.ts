...
export const useMemoriesApi = (): UseMemoriesApiReturn => {
  ...
  const getBaseUrl = useCallback(resolveApiBaseUrl, []);
  ...
  const fetchMemories = useCallback(async (
    ...
    try {
      ...
      const response = await axios.post<ApiResponse>(
        `${baseUrl}/api/v1/memories/filter`,
        {
          user_id: user_id,
          page: page,
          size: size,
      ...
      const adaptedMemories: Memory[] = response.data.items.map((item: ApiMemoryItem) => {
        const createdAtSource = item.created_at ?? item.metadata_?.created_at ?? item.metadata_?.timestamp;
        const createdAtMs = createdAtSource ? Date.parse(createdAtSource) : NaN;
  ...
  const createMemory = async (text: string): Promise<void> => {
    try {
      const repo = process.env.NEXT_PUBLIC_DEFAULT_REPO?.trim() || "agent-swarm-mcp";
      const baseUrl = getBaseUrl();
      const memoryData = {
        repo,
        lesson: text,
      ...
      await axios.post<ApiMemoryItem>(
  ...
  const deleteMemories = async (memory_ids: string[]) => {
    try {
      const baseUrl = getBaseUrl();
      await axios.delete(`${baseUrl}/api/v1/memories/`, {
  ...
  const fetchMemoryById = async (memoryId: string): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.get<SimpleMemory>(
        `${baseUrl}/api/v1/memories/${memoryId}?user_id=${user_id}`
      );
      setIsLoading(false);
  ...
  const fetchAccessLogs = async (memoryId: string, page: number = 1, pageSize: number = 10): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.get<AccessLogResponse>(
        `${baseUrl}/api/v1/memories/${memoryId}/access-log?page=${page}&page_size=${pageSize}`
      );
      setIsLoading(false);
  ...
  const fetchRelatedMemories = async (memoryId: string): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.get<RelatedMemoriesResponse>(
        `${baseUrl}/api/v1/memories/${memoryId}/related?user_id=${user_id}`
      );
  ...
  const updateMemory = async (memoryId: string, content: string): Promise<void> => {
    ...
    try {
      const baseUrl = getBaseUrl();
      await axios.put(`${baseUrl}/api/v1/memories/${memoryId}`, {
        memory_id: memoryId,
  ...
  const updateMemoryState = async (memoryIds: string[], state: string): Promise<void> => {
    ...
    try {
      const baseUrl = getBaseUrl();
      await axios.post(`${baseUrl}/api/v1/memories/actions/pause`, {
